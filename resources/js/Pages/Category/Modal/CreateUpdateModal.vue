...
import { useForm } from '@inertiajs/vue3';
    ...
    function submit(state, id){
        state == 'create' ? form.post(route('category.store'), {
            onSuccess: () => {
                emit('modal_close');
            }
        }) : form.patch(route('category.update', id), {
            onSuccess: () => {
                emit('modal_close');
            }
        });
...
<template>
    <div id="category_modal" tabindex="-1" aria-hidden="true" class="flex bg-black bg-opacity-50 overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full inset-0 h-full">
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-800">
                <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-900 bg-purple-500 dark:bg-purple-600">
                    <h3 class="text-lg font-semibold text-white dark:text-gray-200">
                        {{state == 'create' ? 'Create New' : 'Edit'}} Product Category
                    </h3>
                ...
                <form class="p-4 md:p-5" @submit.prevent="submit(state, category ? category.id : null)">
