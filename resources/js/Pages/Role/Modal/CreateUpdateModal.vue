...
import { useForm } from '@inertiajs/vue3';
    ...
    function submit(state, id){
        state == 'create' ? form.post(route('role.store'), {
            onSuccess: () => {
                emit('modal_close');
            }
        }) : form.patch(route('role.update', id), {
            onSuccess: () => {
                emit('modal_close');
            }
        });
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-800">
                <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-900 bg-purple-500 dark:bg-purple-600">
                    <h3 class="text-lg font-semibold text-white dark:text-gray-200">
                        {{state == 'create' ? 'Create New' : 'Edit'}} Role
                    </h3>
                ...
                <form class="p-4 md:p-5" @submit.prevent="submit(state, role ? role.id : null)">
